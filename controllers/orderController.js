...
function postCheckout(req, res) {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ success: false, message: "Dati cliente incompleti o mancanti." });
  }
  ...
  connection.query(sqlCheck, [productIds], (err, winesList) => {
    ...
    for (const item of cart_items) {
      ...
      if (!wine || wine.stock_quantity < item.quantity) {
        return res.status(400).json({
          message: `Stock insufficiente per ${
            wine ? wine.product_name : "ID " + item.product_id
          }`,
        });
      }
      // prendo il prezzo (promo o normale) dal DB
      const realPrice =
        wine.promotion_price !== null ? wine.promotion_price : wine.price;
    ...
    if (discount_code) {
      ...
      connection.query(sqlCoupon, [discount_code], (err, couponResults) => {
        if (err)
          return res
            .status(500)
            .json({ success: false, message: "Errore coupon" });
        ...
        if (!coupon) {
          return res
            .status(400)
            .json({ message: "Codice sconto non esistente o non valido" });
        }
        if (
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
      ...
      connection.query(sqlCustomer, customerValues, (err, customerResult) => {
        ...
        connection.query(
          sqlOrder,
          [customerId, price, finalShippingFee || 0],
          (err, orderResult) => {
            if (err)
              return res.status(500).json({
                success: false,
            ...
            itemsForCart.forEach((wine) => {
              ...
              connection.query(
                sqlCartItem,
                [orderId, wine.product_id, wine.quantity, wine.price],
                (err) => {
                  if (err && !errorSent) {
                    errorSent = true;
                    return res.status(500).json({
          ...
          AND stock_quantity >= ? `;
                  ...
                  connection.query(
                    sqlUpdateStock,
                    [wine.quantity, wine.product_id, wine.quantity],
                    (err, result) => {
                      if (err && !errorSent) {
                        errorSent = true;
                        return res.status(500).json({
...
function validateCoupon(req, res) {
  ...
  if (!discount_code) {
    return res
      .status(400)
      .json({ success: false, message: "Inserisci un codice" });
  }
  ...
  connection.query(sqlCoupon, [discount_code], (err, results) => {
    ...
    if (total_amount < coupon.min_spending) {
      return res.status(400).json({
        success: false,
        message: `Spesa minima per lo sconto non raggiunta. Aggiungi ${(
          coupon.min_spending - total_amount
        ).toFixed(2)}€ per usare questo codice.`,
      });
    }
